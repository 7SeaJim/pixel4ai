...
> **本项目已于 2026-09-15 归档，不再开发。** 原因、归档时的状态和恢复方法见 [ARCHIVE.md](ARCHIVE.md)。下面的说明保留归档时的原样。
...
- 可选字段 `ramps`：色阶，每组是同一种材质从暗到亮的一串调色板字符，比如 `["JjHh", "Kk"]`。`shade` 靠它知道「暗一档」是哪个颜色；不写这个字段的文件照常可用。
...
```bash
pxl 画.pxl horizon 150 160 k --fill k                       # 起伏的山脊并向下填满（默认起伏约为画布高度的 6%，--seed 换形状）
pxl 画.pxl curve 0 205 70 198 150 210 255 202 H --fill H    # 经过这些点的平滑曲线，向下填充；回显高度剖面
pxl 画.pxl fill 120 80 s --boundary k --closed              # 跨过杂色填到边界字符 k 为止；没围住就报错、不修改
```

构图自检（只读）：

```bash
pxl 画.pxl inspect           # 近似直线的交界、没有过渡带的色带、大块平涂
pxl 画.pxl inspect --object 133 140 228 186 --object 104 165 112 195   # 另外检查这些物体底边下方有没有接触阴影
pxl 画.pxl view --outline    # 只显示材质交界
```

画布在 64×64 到 512×512 之间时，`apply` 结束后会自动做一次检查，有问题才输出一行「构图检查：⚠ …」；加 `--no-lint` 可以关闭。

画阴影、投影、受光面（先声明色阶，再按形状压暗或提亮）：

```bash
pxl 画.pxl ramp suggest                                     # 按色相给出色阶分组建议
pxl 画.pxl ramp set JjHh Kk                                 # 声明色阶，每组从暗到亮
pxl 画.pxl shade ellipse 120 196 210 206 --soft 2           # 椭圆范围压暗 1 档，边缘渐隐
pxl 画.pxl shade poly 180 200 240 204 250 214 176 210 --soft 3 --except Oo   # 多边形投影：地面上的一切都压暗，排除房子自己的颜色
pxl 画.pxl shade ellipse 120 196 210 206 poly 180 200 240 204 250 214 176 210   # 多个形状合并后只处理一次，重叠处不会重复压暗
pxl 画.pxl shade ellipse 120 196 210 206 --extend           # 色阶已到最暗时，自动补一个略偏冷的更暗颜色
```
